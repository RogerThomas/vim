...
syn region alphElseEnd transparent matchgroup=luaRepeat start="\<else\>" end="\<end\>"me=e-2 contains=TOP
syn region alphTry transparent matchgroup=luaRepeat start="\<try\>" end="\<else\>"me=e-2 contains=TOP nextgroup=alphElseEnd skipwhite skipempty

syn keyword luaFor contained containedin=luaFor in

" other keywords
syn keyword luaStatement return var break with
...
syn keyword luaFunc assert collectgarbage dofile error next bitset debug realvector array loop
syn keyword luaFunc print rawget rawset tonumber tostring type _VERSION raise
